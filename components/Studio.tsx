...
import { connectDraft, forgetProject, mergeProjects, prepareDraftConnection, readLocalProjects, saveLocalSnapshot } from '@/lib/browser-drafts';
import projectStyles from './ProjectPicker.module.css';
...
export default function Studio({ authConfigured }: { authConfigured: boolean }) {
  ...
  const [modal, setModal] = useState<'brief' | 'settings' | 'projects' | 'delete-project' | 'help' | null>(null), [brief, setBrief] = useState(initialBrief), [name, setName] = useState('A new perspective');
    ...
    return subscribeToProject({
      ...
      onSnapshot: next => {
        ...
        if (modelChanged) {
          ...
          if (!current.design && updated.design) setNotice('Your first draft is ready. Open Design to inspect it or Files to export it while the team continues.');
        }
      },
    });
  }, [snapshot?.project.id]);
  ...
  async function deleteProject() {
    ...
    if (!target) return;
    if (!target.id.startsWith('local_')) await api(`/projects/${target.id}`, 'DELETE');
    const removed = forgetProject(target.id);
    setProjects(current => current.filter(project => project.id !== target.id && !removed.includes(project.id)));
    const currentId = snapshotRef.current?.project.id;
    if (currentId === target.id || (currentId && removed.includes(currentId))) {
      projectRequest.current++;
      snapshotRef.current = null; setSnapshot(null); setSample(true); setDesign(exampleDesign());
      setSelected(null); setPrevious(null); setCompare(false); setFromPresentation(false);
      setDesktopAgent(null); setViewerIdentity(value => value + 1);
      setTranscripts([]); setReceivedTurns([]); setInput('');
      messageRetry.current = null; inputClarification.current = null;
      chooseRoom('reception'); setMode('office'); setNavigation('walk');
    }
    setDeleteTarget(null); setModal('projects');
    notify(target.id.startsWith('local_') ? 'Browser draft deleted from this device. Any connected server project is unchanged.' : 'Project removed from your workspace. Server files are retained for recovery.');
  }
  ...
  return <main ref={fullscreen.ref} className="studio-shell" data-fullscreen={fullscreen.isFullscreen}>
    ...
    {modal && <div className="modal-backdrop" onClick={() => setModal(null)}><section className="modal" role="dialog" aria-modal="true" aria-label={modal === 'brief' ? 'Project brief' : modal} onClick={e => e.stopPropagation()}><button className="modal-close icon-button" aria-label="Close dialog" onClick={() => setModal(null)}><X size={20} /></button>
      ...
      {modal === 'projects' && <><span className="eyebrow">YOUR WORK</span><h2>Projects in the studio.</h2>{projects.length ? projects.map(p => <div className={projectStyles.row} key={p.id}><button className="file-row" disabled={busy} onClick={() => void action(async () => { await loadProject(p.id); setModal(null); })}><FolderOpen size={20} /><span><strong>{p.name}</strong><small>{p.id.startsWith('local_') ? 'Browser draft · this device' : `Saved online · revision ${p.revision} · ${p.status}`}</small></span><ArrowRight size={17} /></button><button type="button" className={projectStyles.delete} disabled={busy} aria-label={`Delete ${p.name}`} onClick={() => { setDeleteTarget(p); setModal('delete-project'); }}>Delete</button></div>) : <p>No saved projects yet. Bring your first idea to the table.</p>}<button className="button full" disabled={busy} onClick={() => { setName('A new perspective'); setBrief(''); setModal('brief'); }}><Plus size={16} />New project</button></>}
      {modal === 'delete-project' && deleteTarget && <><span className="eyebrow">DELETE PROJECT</span><h2>Delete “{deleteTarget.name}”?</h2><p>{deleteTarget.id.startsWith('local_') ? 'This removes this browser draft and its connection details from this device. It cannot be undone here. Any connected server project stays saved online.' : 'This removes the project from your workspace and clears its linked drafts on this device. Saved server files are retained for recovery; this is not a permanent file purge.'}</p>{!deleteTarget.id.startsWith('local_') && <p className="fine-print">Stop this project’s work and end its live voice call first. Your API key and recorded usage are not deleted.</p>}<div className="modal-actions"><button className="button subtle" disabled={busy} onClick={() => { setDeleteTarget(null); setModal('projects'); }}>Cancel</button><button className={`button ${projectStyles.confirm}`} disabled={busy} onClick={() => void action(deleteProject)}>{busy ? 'Deleting…' : 'Delete project'}</button></div></>}
