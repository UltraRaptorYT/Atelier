...
export default function Studio({ authConfigured }: { authConfigured: boolean }) {
  ...
  const [desktopAgent, setDesktopAgent] = useState<AgentId | null>(null), [desktopMessage, setDesktopMessage] = useState('');
  const [workstationView, setWorkstationView] = useState<'desktop' | 'activity' | 'output' | 'meeting'>('desktop');
  const desktopRequest = useRef(0);
  ...
  useEffect(() => {
    ...
    const key = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { setModal(null); setDesktop(null); setDesktopAgent(null); }
  ...
  async function openWorkstation(target: AgentId) {
    ...
    try {
      const result = await api<{url:string}>(`/projects/${projectId}/desktop/${target}`, 'POST', {});
  ...
  return <main className="studio-shell">
    ...
    {desktopAgent && <div className="modal-backdrop"><section className="desktop-modal" role="dialog" aria-modal="true" aria-label="Live workstation"><header><h2>{workstationView === 'meeting' ? 'Team meeting' : `${agents[desktopAgent].name}’s workstation`}</h2><button className="icon-button" aria-label="Close desktop" onClick={() => { desktopRequest.current++; setDesktop(null); setDesktopAgent(null); }}><X /></button></header>
      <nav className="workstation-people" aria-label="Workstation views"><button className={workstationView === 'meeting' ? 'selected' : ''} onClick={() => setWorkstationView('meeting')}>Meeting table</button>{(Object.keys(agents) as AgentId[]).map(id => <button key={id} className={desktopAgent === id && workstationView !== 'meeting' ? 'selected' : ''} onClick={() => { if (id === desktopAgent) setWorkstationView('desktop'); else void openWorkstation(id); }}><span style={{background:agents[id].color}}/>{agents[id].name.split(' ')[0]}</button>)}</nav>
      {workstationView !== 'meeting' && <nav className="workstation-tabs" aria-label="Computer content">{(['desktop','activity','output'] as const).map(view => <button key={view} aria-pressed={workstationView === view} onClick={() => setWorkstationView(view)}>{view === 'desktop' ? 'Live desktop' : view === 'activity' ? 'Tool activity' : 'Saved outputs'}</button>)}<small>Voice follows your position in the studio.</small></nav>}
      {desktop && <iframe style={{display:workstationView === 'desktop' ? undefined : 'none'}} src={desktop} title="Authenticated agent desktop" allow="clipboard-read; clipboard-write" sandbox="allow-scripts allow-same-origin" referrerPolicy="no-referrer" />}
      {workstationView === 'desktop' && !desktop && <div className="files-panel"><p role="status">{desktopMessage}</p><button className="button" onClick={() => void openWorkstation(desktopAgent)}>Reconnect computer</button>{snapshot?.artifacts.filter(a => a.kind === 'desktop-preview' && a.name.startsWith(desktopAgent)).slice(0,1).map(a => <img key={a.id} src={`/api/studio/projects/${projectId}/artifacts/${a.id}?inline=1`} alt="Last saved workstation checkpoint" style={{width:'100%'}} />)}<h3>Real activity</h3>{snapshot?.events.filter(e => e.agent === desktopAgent).slice(-8).reverse().map(e => <p key={e.id}>{e.message}</p>)}</div>}
      {workstationView === 'activity' && <div className="workstation-content"><h3>Actual tool activity</h3>{snapshot?.tasks.filter(t => t.agent === desktopAgent).slice(0,3).map(t => <p key={t.id}><strong>{t.title}</strong> · {t.status.replaceAll('_',' ')}<br/>{t.detail}</p>)}{snapshot?.events.filter(e => e.agent === desktopAgent).slice(-30).reverse().map(e => <article key={e.id}><small>{new Date(e.createdAt).toLocaleTimeString()} · {e.type.replaceAll('_',' ')}</small><p>{e.message}</p></article>)}{!projectId && <p>Create a project to watch live work.</p>}</div>}
      {workstationView === 'output' && <div className="workstation-content"><h3>Saved project outputs</h3>{snapshot?.artifacts.length ? snapshot.artifacts.map(a => <a className="file-row" key={a.id} href={`/api/studio/projects/${projectId}/artifacts/${a.id}`} download><FileText size={18}/><span><strong>{a.name}</strong><small>{a.kind} · revision {a.revision}</small></span><ArrowDownToLine size={16}/></a>) : <p>Outputs will appear after your team saves its work.</p>}</div>}
      {workstationView === 'meeting' && <div className="workstation-content"><h3>The shared brief</h3><p>{snapshot?.project.brief.summary || 'Tell the team what you would like to build.'}</p>{snapshot?.project.brief.questions.map(q => <p key={q}><strong>Question:</strong> {q}</p>)}{snapshot?.events.filter(e => ['meeting_started','meeting_ended','agent_message','clarification_requested','clarification_received'].includes(e.type)).slice(-24).reverse().map(e => <article key={e.id}><strong style={{color:e.agent ? agents[e.agent].color : undefined}}>{e.agent ? agents[e.agent].name : 'Team'}</strong><p>{e.message}</p></article>)}</div>}
      </section></div>}
