...
export default function Studio({ authConfigured }: { authConfigured: boolean }) {
  ...
  async function startRun(kind: 'generate' | 'render') {
    ...
    if (localProject && sitesHosted && snapshot) {
      ...
      await api(`/projects/${saved.id}/runs`, 'POST', { kind, operationId: crypto.randomUUID(), baseRevision: saved.revision });
      await loadProject(saved.id); setTab('activity'); notify('Your brief is saved online. Your principal is preparing the team briefing.');
      return;
    }
