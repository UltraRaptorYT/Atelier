...
New runs without an existing design now use an explicit draft-first graph: Architect draft and Designer visual direction in parallel, then architectural detail and interior placement in parallel, then combined review. The first draft's six-minute work allowance includes required inspection; visual direction has two minutes. The draft is committed and announced before refinement, so the existing viewer and exports can use it immediately. Batch failure, ownership, cancellation and revision fences remain unchanged. The seated office avatars now bend at hips/knees and place their thighs on the chair cushions with feet above the floor. These changes were typechecked but not exercised through paid providers or browser visual testing.
...
Normal computer caps remain two global desktops, 900 seconds per lease, 3,600 user seconds per day, and the configured global monthly allowance. An explicit local developer run/day override can add up to 3,600 daily seconds, retaining all recorded charges and monthly limits. Task work has 12 minutes within a 13-minute checkpoint; file proposals allow 24 tool turns including inspection and later submission. Failed authoring attempts preserve a bounded unpublished draft when available, and application-authored proposal errors appear in Activity. Unused reserved time is refunded only after confirmed shutdown; uncertain or legacy usage retains its conservative charge. A deterministic single-element color patch needs no editing desktop, while its model review still uses the Critic workstation. Long work or retries can still exhaust the allowance. Image requests have a separate allowance and do not reserve desktops. See [budget limits](../shared/budget.ts), [budget accounting](../worker/src/budget.ts) and [desktop lifecycle](../worker/src/desktop.ts).
...
Voice save hardening also accepts short first requirements and preserves pending clarification answers when appending unrelated requirements. The waiting brief snapshot and question version update atomically. Voice tool failures emit a safe Activity error; success is not manufactured for an unsaved result. Routing uses low reasoning effort under the existing 1,800-token cap. The complete spoken save/start flow still needs live deployment acceptance; typechecking does not prove provider latency or spoken behavior.

Run admission now has a structured RPC result for API/voice callers, avoiding lost `HttpError` status across the coordinator boundary. Every newly admitted job uses the durable dispatch outbox: retry the saved Workflow ID after uncertain creation, retain queued state, and respect cancellation and current generation gates. Production was inspected read-only on 2026-09-13: migrations through 0007 were present, no active jobs were recorded, and the latest stored failed design attempt reached specialist execution before a `Connection error.` These observations do not verify the new code on production or establish the cause of every queue error.
