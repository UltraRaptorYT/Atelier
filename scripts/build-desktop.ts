...
if (await Template.exists('atelier-desktop', { apiKey: process.env.E2B_API_KEY })) {
  console.log('Atelier desktop template already exists. Running the benchmark is sufficient.');
} else {
  await Template.build(template, 'atelier-desktop', { apiKey: process.env.E2B_API_KEY, cpuCount: 8, memoryMB: 8192, minFreeDiskMb: 2048, onBuildLogs: defaultBuildLogger({ minLevel: 'info' }) });
}
