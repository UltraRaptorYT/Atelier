export type Secrets = { CLERK_SECRET_KEY?: string; CLERK_JWT_KEY?: string; E2B_API_KEY?: string; KEY_ENCRYPTION_KEYS?: string; OPENAI_API_KEY?: string };
export type Bindings = { [K in keyof Env]: Env[K] extends string ? string : Env[K] } & Secrets;
export type RunParams = { projectId: string; userId: string; runId: string; kind: 'generate' | 'change' | 'render' | 'image'; baseRevision: number; instruction?: string; agent?: import('../../shared/design').AgentId; elementId?: string | null; referenceArtifactId?: string | null };
