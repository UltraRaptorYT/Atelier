...
describe('design generation reasoning configuration', () => {
  it('defaults to high reasoning and rejects unsupported effort before a request', () => {
    expect(designReasoningEffort({ OPENAI_REASONING_EFFORT: '' })).toBe('high');
  ...
  it('ships Astra high consistently across deployment environments', () => {
    const config = JSON.parse(readFileSync('worker/wrangler.jsonc', 'utf8'));
    for (const name of ['local', 'staging', 'production']) {
      expect(config.env[name].vars).toMatchObject({ OPENAI_MODEL: 'gpt-6-astra', OPENAI_REASONING_EFFORT: 'high' });
    }
  });
});
