...
Structured agent calls have a 64,000-token allowance per response, including reasoning, while retaining the full agent guidance and existing time limits. **Retry team briefing** uses the same project's saved brief and answers in a new attempt. See [model settings and diagnostics](ENVIRONMENT.md#4-models-and-feature-gates-workerwranglerjsonc).
