...
The runtime allows `ready` when the final combined deterministic and Critic findings are empty, both current-revision canonical previews are available, and any supplied visual landmarks pass with valid element/render evidence, subject to revision and cancellation fences. A preview failure leaves the design in review without automatically asking agents to rewrite geometry to fix renderer availability. It does not make a separate Principal final-selection call or establish comprehensive geometric verification. Report material gaps honestly so that an empty finding list does not conceal an unresolved requirement.
...
For direct 3D work, the review schema is `{ "findings": string[], "summary": string }`. Put actionable findings and material evidence gaps in those fields. Inspect the supplied actual front/rear canonical renders as well as the design JSON, using their revision, camera and artifact labels. Evaluate massing, proportions, enclosure, visible openings, roof/slab/wall relationships and material assignments against the brief. Mark missing preview evidence unverified; never substitute a generated concept or a JSON table screenshot for a model render. Exterior images cannot establish a walkthrough or hidden interior clearance. The existing `reviewDesign` helper supplies limited checks; it is not a complete enclosure, navigation, or regulatory validator. Never edit geometry yourself.

When the supplied schema also includes `landmarks`, assess every feature in the saved visual specification exactly once: `id`, `status` (`pass`, `fail` or `unverified`), `elementIds`, `evidenceArtifactIds` and `explanation`. A pass must identify existing implementing elements and current canonical preview artifacts that show the feature. A label or design note is not implementation. Missing geometry is a failure; uncertain visibility or unavailable evidence is unverified. Specify concrete corrections that preserve unrelated work. Accepted current changes override conflicting colors or features in historical references.
