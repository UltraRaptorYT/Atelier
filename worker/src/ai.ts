    ...
    for (const call of calls) {
      ...
      try {
        ...
        if (call.name === "read_design")
          output = await context.desktop.files.read(
            "/home/user/project/design.json",
          );
          ...
          const { code } = z
            .object({ code: z.string().max(12000) })
            .parse(args);
          await context.desktop.files.write(
            "/home/user/project/agent_task.py",
            code,
          );
          const execution = await context.desktop.commands.run(
            "timeout 45s python3 /home/user/project/agent_task.py",
            { timeoutMs: 50000 },
          );
        ...
        } else if (call.name === "desktop_click") {
          const { x, y } = z
            .object({
            ...
            .parse(args);
          await context.desktop.leftClick(x, y);
          output = "Clicked.";
        } else if (call.name === "desktop_type") {
          const { text } = z.object({ text: z.string().max(4000) }).parse(args);
          await context.desktop.write(text);
          output = "Text entered.";
        } else if (call.name === "register_blender_asset") {
          const args2 = z
            .object({
              blendFile: z.string().regex(/^[a-zA-Z0-9_-]+\.blend$/),
              name: z.string().min(1).max(100),
            })
            .parse(args);
          const assetId = `asset_${crypto.randomUUID().replaceAll("-", "")}`;
          const compiled = await context.desktop.commands.run(
            `timeout 90s blender --background --python-exit-code 1 --python /home/user/project/blender_asset.py -- --input /home/user/project/${args2.blendFile} --output /home/user/project/output/${assetId}.glb`,
            { timeoutMs: 95000 },
          );
          if (compiled.exitCode !== 0)
            throw new Error("The geometry asset could not be compiled.");
          const revision =
            (
          ...
          const artifactId = await artifact(
            ...
            await context.desktop.files.read(
              `/home/user/project/output/${assetId}.glb`,
              { format: "bytes" },
            ),
            "model/gltf-binary",
          );
          await artifact(
            ...
            revision,
            await context.desktop.files.read(
              `/home/user/project/${args2.blendFile}`,
              { format: "bytes" },
            ),
