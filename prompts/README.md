...
[team.ts](../worker/src/team.ts) executes validated plans using [collaboration.ts](../shared/collaboration.ts): 2–8 tasks in an acyclic dependency graph, up to two different specialists per batch, one task per specialist at a time. New designs use [draft-plan.ts](../shared/draft-plan.ts): an inspected Architect draft and Designer visual study run together, then architectural refinement and interior placement run together against the committed draft, followed by combined review. The six-minute draft pass defers optional Blender GUI polishing without waiving file inspection/submission. Existing-design changes and bounded correction plans remain Principal-authored. A final Critic task must depend transitively on every other task. Independent existing-design architecture and finish proposals can run together.
...
- Clarification questions leave a blocked Principal task and a persistent, versioned question record. Text and voice share question/brief/answer/change routing. Actual answers continue the already-authorized briefing once all required questions are answered. Extra pre-design requirements may be saved while questions are open without answering or cancelling them; refresh context before using the new question version. Voice saves only new user requirements, reports specific failures, and must not start with an older brief after a failed save.
...
- An accepted queued request is not a running specialist. The coordinator retries unconfirmed Workflow dispatch using the same saved job ID; do not ask for another start approval or claim the team is working before its persisted status confirms it. Specific admission failures are returned to both API and voice callers.
