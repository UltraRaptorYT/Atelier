...
// Keep headroom for the full agent instructions without lowering design effort.
...
export function designReasoningEffort(env: Pick<Bindings, 'OPENAI_REASONING_EFFORT'>) {
  const parsed = DesignEffortSchema.safeParse(env.OPENAI_REASONING_EFFORT?.trim() || 'max');
