...
const localProjectsKey = 'atelier-local-projects';
...
function readLocalProjects(): Snapshot[] {
  try { return JSON.parse(localStorage.getItem(localProjectsKey) || '[]') as Snapshot[]; } catch { return []; }
}
function saveLocalSnapshot(snapshot: Snapshot) {
  const projects = readLocalProjects().filter(item => item.project.id !== snapshot.project.id);
  localStorage.setItem(localProjectsKey, JSON.stringify([snapshot, ...projects]));
}
...
export default function Studio({ authConfigured }: { authConfigured: boolean }) {
  ...
  async function createProject() {
    ...
    try {
      const p = await api<Project>('/projects', 'POST', { name, brief: b });
      await loadProject(p.id);
      notify('Project saved. Start a team briefing when you are ready.');
    } catch (error) {
  ...
  async function createVoiceProject() {
    ...
    try {
      const p = await api<Project>('/projects', 'POST', { name, brief: b });
      await loadProject(p.id); notify('Project ready. Click Live voice to tell your principal what you want to build.');
    } catch (error) {
      if (authConfigured) throw error;
      const next = newLocalSnapshot(name.trim(), b);
    ...
    chooseRoom('principal'); setModal(null);
  }
  async function startRun(kind: 'generate' | 'render') {
    if (!projectId) { setModal('brief'); return; }
    if (localProject) { notify('This browser-saved draft is ready. Live AI and E2B workstations need the authenticated studio service.'); return; }
