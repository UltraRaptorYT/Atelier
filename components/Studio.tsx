...
export default function Studio({ authConfigured }: { authConfigured: boolean }) {
  ...
  async function openWorkstation(target: AgentId) {
    setDesktopAgent(target); setDesktop(null); setDesktopMessage('Connecting to the real E2B computer...');
    if (document.pointerLockElement) document.exitPointerLock();
    ...
    } catch (error) { setDesktopMessage(error instanceof Error ? error.message : 'Computer unavailable. Saved activity is shown below.'); }
  }
