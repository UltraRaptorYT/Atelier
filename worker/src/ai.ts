...
export async function modelJSON<T>(env: Bindings, owner: string, agent: AgentId, prompt: string, schema: z.ZodType<T>, context?: { desktop: Sandbox; projectId: string; taskId: string; design: Design | null; communications?: Array<{ target: AgentId; message: string }> }, images: string[] = []): Promise<T> {
  ...
  const tools: OpenAI.Responses.Tool[] = context ? [
    ...
    { type: 'function', name: 'desktop_move', description: 'Move the actual mouse to inspect or target a visible UI control. Never move it just to simulate activity.', parameters: {type:'object',properties:{x:{type:'integer'},y:{type:'integer'}},required:['x','y'],additionalProperties:false}, strict:true },
    { type: 'function', name: 'desktop_key', description: 'Press a key or keyboard shortcut in the real focused app, for example Return or ctrl+s.', parameters:{type:'object',properties:{keys:{type:'array',items:{type:'string'},maxItems:3}},required:['keys'],additionalProperties:false},strict:true},
  ...
  for (let step = 0; step < 12; step++) {
    ...
    for (const call of calls) {
      ...
      try {
        ...
        if (call.name === 'read_design') { await context.desktop.launch('mousepad', '/home/user/project/design.json'); output = await context.desktop.files.read('/home/user/project/design.json'); }
      ...
      } catch (error) { const details = error && typeof error === 'object' && 'stderr' in error && typeof error.stderr === 'string' ? error.stderr.slice(0, 2000) : ''; output = `Tool failed. ${details || 'Inspect the current state and choose a recoverable next action.'}`; }
